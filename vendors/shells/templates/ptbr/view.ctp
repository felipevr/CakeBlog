...
<?php
	...
	echo "\t\t<li><?php echo \$this->Form->postLink(__('Apagar " . $singularHumanName . "'), array('action' => 'delete', \${$singularVar}['{$modelClass}']['{$primaryKey}']), null, __('Tem certeza que você quer apagar # %s?', \${$singularVar}['{$modelClass}']['{$primaryKey}'])); ?> </li>\n";
...
foreach ($relations as $alias => $details):
	...
	?>
<div class="related">
	<h3><?php echo "<?php echo __('Relacionado " . $otherPluralHumanName . "');?>";?></h3>
	<?php echo "<?php if (!empty(\${$singularVar}['{$alias}'])):?>\n";?>
...
echo "\t<?php
		...
		echo "\t\t<tr>\n";
			...
			echo "\t\t\t\t<?php echo \$this->Form->postLink(__('Apagar'), array('controller' => '{$details['controller']}', 'action' => 'delete', \${$otherSingularVar}['{$details['primaryKey']}']), null, __('Tem certeza que você quer apagar # %s?', \${$otherSingularVar}['{$details['primaryKey']}'])); ?>\n";
			echo "\t\t\t</td>\n";
		echo "\t\t</tr>\n";
